...
const { cart } = router.query;
  ...
  const getData = async () => {
    try {
      const res = await axios.get(
        `http://localhost:1000/ecomm-backend/cart/${cart}`
      );
  ...
  const btnSubmit = async (e) => {
    ...
    try {
      ...
      let res;
      if(cart) {
         res = await axios.put(`http://localhost:1000/ecomm-backend/cart/${cart}`, 
         {uid,pid}
        );
      } else {
         res = await axios.post("http://localhost:1000/ecomm-backend/cart", 
            {uid,pid}
        );
      }
      if (res?.status === 200) {
