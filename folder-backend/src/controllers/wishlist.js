...
const getData1=async (req,res)=>{
    ...
    try{
        ...
        const q="SELECT * FROM `wishlist` WHERE wid=?";
...
const insertData = async (req, res) => {
  ...
  try {
    const q = "INSERT INTO `wishlist`(`uid`, `pid`) VALUES (?)";
    const values = [
      req?.body?.uid,
      req?.body?.pid,
    ];
...
const updateData = async (req, res) => {
    ...
    try {
      const q = "UPDATE `wishlist` SET `uid`=?,`pid`=? WHERE wid=?";
      const values = [
        req?.body?.uid,
      req?.body?.pid,
      ];
  ...
  const deleteData = async (req, res) => {
    ...
    try {
      const q = "DELETE FROM `wishlist` WHERE wid=?";
      db.query(q, req?.params?.id, async (err, data) => {
        if (err) {
          return await res?.status(400)?.json({
            status: 400,
