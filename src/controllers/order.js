...
const getData1=async (req,res)=>{
    ...
    try{
        ...
        const q="SELECT * FROM `order` WHERE oid=?";
...
const insertData = async (req, res) => {
  ...
  try {
    const q = "INSERT INTO `order`(`uid`,`oamt`,`odate`) VALUES (?)";
    const values = [
      req?.body?.uid,
      // req?.body?.cartid,
      req?.body?.oamt,
...
const updateData = async (req, res) => {
    ...
    try {
      const q = "UPDATE `order` SET `uid`=?,`oamt`=?.,`odate`=? WHERE oid=?";
      const values = [
        req?.body?.uid,
      // req?.body?.cartid,
      req?.body?.oamt,
  ...
  const deleteData = async (req, res) => {
    ...
    try {
      const q = "DELETE FROM `order` WHERE oid=?";
      db.query(q, req?.params?.id, async (err, data) => {
        if (err) {
          return await res?.status(400)?.json({
            status: 400,
